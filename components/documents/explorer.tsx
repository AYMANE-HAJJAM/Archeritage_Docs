...
import { ArrowDownToLine, ArrowLeft, Check, ChevronRight, Eye, FileText, FolderClosed, FolderPlus, ImageIcon, LayoutGrid, List, Loader2, MoreHorizontal, MoveRight, Pencil, Trash2, Upload, X } from "lucide-react";
...
export function Explorer({
  ...
  function actionMenu() {
    ...
    return createPortal(<div ref={menuRef} role="menu" className="fixed z-[100] min-w-40 rounded-sm border bg-background p-1 text-sm shadow-lg" style={{ top: menuPosition.top, left: menuPosition.left }}>
      ...
      <button type="button" role="menuitem" className="flex w-full items-center gap-2 rounded-sm px-2.5 py-2 text-left text-destructive hover:bg-muted" onClick={() => run(() => openModal({ action: "delete", target }))}><Trash2 className="size-3.5" />Supprimer</button>
    </div>, document.body);
  }
